...
void NFC_user_Handler(u8 ch)
{
	...
	else
	{
		...
		if (rx_cnt >= 25)
		{
			...
			// 判断 UID 特定字节（[19]~[22]），三张预设卡
			NFC_Card_Index = 0;
			if (rx_buf[19]==0xB9 && rx_buf[20]==0x80 && rx_buf[21]==0x06 && rx_buf[22]==0x85)
			{
				NFC_Card_Index = 1;
			}
			else if (rx_buf[19]==0x50 && rx_buf[20]==0x84 && rx_buf[21]==0xFC && rx_buf[22]==0x23)
			{
				NFC_Card_Index = 2;
			}
			else if (rx_buf[19]==0x40 && rx_buf[20]==0x74 && rx_buf[21]==0x80 && rx_buf[22]==0x23)
			{
				NFC_Card_Index = 3;
			}

			if (NFC_Card_Index != 0)     // 找到预设卡
			{
				NFC_Card_ID[0] = rx_buf[19];
				NFC_Card_ID[1] = rx_buf[20];
				NFC_Card_ID[2] = rx_buf[21];
				NFC_Card_ID[3] = rx_buf[22];
				NFC_find_Card = 1;
			}
			rx_cnt = 0;
		}
