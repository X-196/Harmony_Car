...
//=================== 灯效模式(逐帧非阻塞, 可随时切换) ===================
#define LED_MODE_OFF       0   //关闭(全灭)
#define LED_MODE_HELLO     1   //HELLO: 跑马灯
#define LED_MODE_ALLON     2   //ALL LIGHT: 前白后红
#define LED_MODE_FASTROUND 3   //FASTROUND: 灯光快速流转(多彩)
#define LED_MODE_SLOWROUND 4   //SLOWROUND: 灯光慢速流转(多彩)
#define LED_MODE_FASTBL    5   //FASTBL: 灯光快速交替闪烁(红蓝)
#define LED_MODE_SLOWBL    6   //SLOWBL: 灯光慢速交替闪烁(红蓝)

void led_effect_init(void);          //初始化灯效状态
void led_effect_run(void);           //逐帧驱动, 每帧调用一次(非阻塞)
void led_effect_set_mode(u8 mode);   //设置灯效模式
