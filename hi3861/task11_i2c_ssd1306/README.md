...
与参考版（ASCII 时钟）不同，学生版改为显示中文"鸿蒙先锋号"：

- **初始化**：`SSD1306_Init()` 配置 GPIO9/10 复用为 I2C0、初始化 SSD1306；`SSD1306_CLS()` 清屏；
- **显示中文**：`SSD1306_ShowChinese(24, 1, (uint8_t*)"鸿蒙先锋号")` —— 5 个 16×16 汉字，宽 80px，居中（x=24）放在第 1 行（y=1，对应页 2/3）；
- **新增中文字库** `include/hal_bsp_ssd1306_fonts_cn.h`：`HZ16Char[][3]` 存各字 UTF-8 编码、`HZ16[][32]` 存 16×16 点阵（每个字 32 字节：前 16 字节为上页、后 16 字节为下页，按列存储）；
- **新增显示函数** `src/hal_bsp_ssd1306.c` 里 `SSD1306_ShowChinese()`：按 UTF-8 3 字节匹配 `HZ16Char`，命中后用 `SSD1306_SetPos` + `SSD1306_WiteData` 逐列写入上/下两页（页寻址模式下列自动前进）。

> 说明：原支持包字库 `F6x8/F8X16` 只有 ASCII，无法显示中文；学生版在 BSP 中扩展了 16×16 中文字库与 `SSD1306_ShowChinese()`。
