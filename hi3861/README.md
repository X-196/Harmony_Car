...
| 任务17 | `task17_line_dfs/` | 双探头黑线循迹 + 左/右岔路 DFS 探索 + 死胡同倒车回溯 | ⚠️ 源码整理完成，待目标环境编译与实车标定 |
...
**成果**：✅ 编译成功（两侧：BUILD SUCCESS + Keil 0 Error 0 Warning），**实机验证通过**（2026-09-01，含转向灯标定）。
...
## 扩展：迷宫巡逻（task16_maze_patrol，避障 + 防跌落二合一）

任务24 决策层 + task15 软UART 传输层的迷宫演示强化：纸箱迷宫自主巡逻 **2 分钟**——超声波避障不撞箱，TCRT5000 朝下检测**黑胶带禁区**（与桌沿防跌落同构：读数 ≠ 开机标定的地面电平即危险）。开机自标定地面电平、全程 `hi_get_us()` 实时计时、运动中每 40ms 查红外、演示到时自动停。传输层 GPIO11 软件 UART @9600，**实车 STM32（9-02 已重烧 9600 版）无需重烧**。

- ✅ 编译成功（BUILD SUCCESS，784840 字节，md5 `e44bff236eb19830dcee03e996415e63`），**实机验证待做**（验证清单/微调指引见 README）
- 详见 [`task16_maze_patrol/README.md`](task16_maze_patrol/README.md)。
