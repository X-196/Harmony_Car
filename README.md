...
│   ├── task06_tcrt_timer/   # 任务6：红外对管收发（TCRT5000 + 软件定时器）
│   │   ├── 2.0_TCRT_Timer/       # 工程（TCRT.c + BUILD.gn）
...
└── stm32/                   # STM32 侧 Keil MDK5 工程
    ...
    ├── 7_NFC读卡/            # 任务20：NFC 读卡（PN532 串口指令 + 卡号判定）
...
- **任务20 · 7_NFC读卡**：串口2 控制 PN532 NFC 模块实现"扫描正确的卡片"
  - 发**唤醒指令**（24B）唤醒 PN532（串口2 PA2/PA3，115200），应答丢失自动重发
  - **循环寻卡**：每 200ms 发寻卡指令；串口2 中断逐字节组帧，无卡应答仅 6 字节先导，有卡收满 25 字节判定
  - **卡号判定**：25 字节应答帧内搜索预设卡 UID，命中才触发；串口1 打印收帧与卡号
  - 学生自定义 `FoundCard_Handler`：刷卡后灯**交替亮灭**，不同卡亮不同颜色（红/绿/蓝）
...
- **任务13 · task13_ap3216c**：OpenHarmony 系统驱动实验（I2C 读 AP3216C 光照强度）
  ...
  - 单任务每 1s 采集打印 ir/als/ps 三路数据（ASCII 输出）+ **OLED 实时显示**（复用任务11 SSD1306 驱动，同挂 I2C0）；读寄存器用「写寄存器地址 → I2cRead」两段传输（与 supportPack 一致）
  - ✅ 编译 + 实机验证通过（md5 `5120b40810fb13c6924199e9d9a1a3ff`，数值随光照/接近变化，OLED 亮屏显示）
## 环境与工具链
...
7. 「7_NFC读卡」：上电后自动唤醒 PN532 并循环寻卡；将 NFC 卡放置于小车读卡区域，后灯交替亮灭（不同卡不同颜色），串口(115200)打印 NFC 收帧与卡号。
...
> - [`hi3861/task06_tcrt_timer/README.md`](hi3861/task06_tcrt_timer/README.md) —— 任务6 红外对管收发 + 软件定时器
